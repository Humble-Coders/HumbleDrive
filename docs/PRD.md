...
| D-13 | Admin web app deploys to Vercel | Already in use at Humble Coders; push-to-deploy, free tier is ample. Served on the Vercel default domain for v1 — see D-37 |
...
| D-33 | The supervisor's **live map stays in v1**, and is delivered **entirely within one ticket** (T13) — subscription, marker, and the single RLS policy it needs | Considered cutting it, then kept it. Confining it to one ticket means no other ticket changes: the driver app already records everything, ticket 8 already left a placeholder, and the policy arrives as part of T13's own migration |
...
| D-35 | A supervisor can **force-complete** a trip without a photo, recording who did it and why | Closes the gap created by T14's geofence: a driver who cannot get a usable fix at the destination would otherwise be stuck, and cancelling would falsely mark a delivered consignment as cancelled |
